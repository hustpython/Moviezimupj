...
void Shotinfo::setImage(int scaledwidth,double pos1,double pos2)
{
    if(myimgurls.length()>0)
    {
        ...
        totalheight = pos2scaled + zimucount *zimuheight;
        ...
        for(int i=0;i<myimgurls.length();i++)
        {
            ...
            for(int x = 0;x<img.width();x++)
            {
                for(int y = initialy;y<pos2scaled;y++)
                {
                    ...
                    {
                     lastimg.setPixel(x,(i-1)*(zimuheight)+pos2scaled+y-initialy,qRgb(r,g,b));
                    }
            }
            }
        }
